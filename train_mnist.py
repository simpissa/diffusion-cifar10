...
def load_mnist(batch_size=64, val_split=0.1):
    ...
    Args:
        ...
        val_split (float): Fraction of training data to use for validation (0-1)
    ...
    # Ensure consistent split if val_split > 0
    generator = None
    if val_split > 0:
    ...
    # Create data loaders
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True) # Added num_workers and pin_memory for potential speedup
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True) if val_split > 0 else None
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i, (real_images, _) in pbar:
            ...
            if global_step % vis_interval == 0:
                 save_plots(metrics, plots_dir, filename=f"metrics_epoch_{epoch+1}_step_{global_step}.png")
...
if __name__ == "__main__":
    ...
    LEARNING_RATE = 2e-4  # Adjusted learning rate (common for diffusion)
    ...
    VIS_INTERVAL = 500    # Save metrics plot every N iterations
    SAVE_INTERVAL = 5    # Save checkpoint every N epochs
    OUTPUT_DIR = "diffusion_mnist_output" # Folder for all outputs
    # CHECKPOINT_TO_RESUME = None # Set path like "diffusion_output/checkpoints/checkpoint_epoch_X.pt" to resume
    CHECKPOINT_TO_RESUME = None # Example: "diffusion_mnist_output/checkpoints/checkpoint_epoch_5.pt"

    # --- Load Data ---
    print("Loading MNIST dataset...")
    train_loader, _, test_loader = load_mnist(batch_size=BATCH_SIZE, val_split=VAL_SPLIT)
